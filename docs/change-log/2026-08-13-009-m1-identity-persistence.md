...
- 本记录中的“已完成”仅表示第 009 号代码变更已经结束。其验证以 Mock、SQL 编译、离线迁移和单元测试为主，不能证明 M1 预设功能完成或无冲突。
- 复核发现：`get_current_user()` 会提交共享 Session；会话/用户与角色分步读取可能形成交叉授权快照；登录验证后没有签发前并发复验；客户端地址端口未冻结；当前限流仅为 `(账号, 来源)` 组合维度。现有身份依赖不得接入生产写路由，最新状态以第 010 号记录为准。
