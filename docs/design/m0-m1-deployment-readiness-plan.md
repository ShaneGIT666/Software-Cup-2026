...
| 生产旧入口隔离 | 应用层保护代码已验证；物理退役未完成 | `disabled` 模式统一返回 404，生产拒绝重新启用；旧路由/挂载仍存在，M2/M7 仍需迁移和纵深防御 |
...
- 生产不变量进程内验证：`APP_DATABASE_REQUIRED=false` 不能把 PostgreSQL 降为可选；缺幂等密钥、HTTPS 来源、身份配置或目标关键 contributor 时 `ready=503 DEPENDENCY_UNAVAILABLE`。
...
| P1（写端口已关闭） | M0 原有 outbox 表缺公共 Writer 和版本/请求字段 | M2/M3 直接写 M0 ORM，M4 复制消费者实现 | `OutboxWriter`、不可变结果和 `0005` 已搭建；M4 claim/lease 仍须单独设计，不得直接操作 ORM |
...
- 当前迁移基线为单一 head `20260814_0005`。领域模型和测试可以并行，正式 migration revision 由 M0 指定集成人员基于合并时最新 head 串行生成；任何人不得修改 `0001`～`0005`。
...
以上文件均已创建并通过对应进程内/契约测试，只表示 D1 代码级门槛已搭建；真实 PostgreSQL、代理、Windows Service 和浏览器验收仍未完成。
...
```text
D0 当前代码/进程内基线
  -> D1 M0 readiness + 旧表面 + outbox 公共端口（代码级已验证）
  -> D2 PostgreSQL 16 在线迁移与 M1 集成/并发验收
  -> D2A 允许 M2/M3 后端写路由接入真实 M1
...
- readiness contributor、生产不变量、旧表面保护和 `OutboxWriter` 已搭建并通过进程内测试。
- M1 `AuditWriter` 已改为返回不可变结果，OpenAPI 已声明 Cookie、CSRF、匿名面和权限扩展。
- 新增 M0 后继迁移 `0005`，未修改 `0001`～`0004`；真实在线升级不属于本阶段已验证证据。

### D2：真实 PostgreSQL 验收

1. 创建空的 `*_test` 数据库和最小权限账户。
2. 在线 upgrade 到当前 head `0005`，检查 outbox 新字段、表、种子、唯一约束和审计不可变触发器。
...
- D1 与 D2 通过后，M2/M3 后端写路由可从身份 Mock 切换到真实 `CurrentUser`/`AuditWriter`/`OutboxWriter`；不再错误等待 Windows Service 或 M6 前端。
...
| 模块 | 现在可以并行 | 必须等待 | 禁止事项 |
| --- | --- | --- | --- |
| M0 | D1 公共端口已搭建；可设计 M4 claim 端口 | D2 在线验证由 M7 提供环境 | 不加入领域业务；不修改 `0001`～`0005` |
| M1 | 补 PostgreSQL 集成测试 | M7 PostgreSQL 环境用于在线验收 | 不再扩展私有接口给 M2/M3；不修改 M0 root router/readiness 聚合器 |
| M2 | 领域模型、DTO、Service、存储端口和身份/outbox Mock 测试 | D2 后接真实写路由；迁移最终编号由 M0 集成 | 不扩展旧 `knowledge.py`/静态下载；不导入 M1 ORM/Repository |
| M3 | 设备/流程领域和 Mock 测试 | D2 后接真实身份；迁移最终编号由 M0 集成 | 不默认绑定流程；不修改 M2 表 |
...
D1 已按第 013 号记录完成代码级搭建。下一批主责转为 M7/M1 的 D2 在线验收：

1. M7 建立 PostgreSQL 16 专用 `*_test` 数据库和最小权限账户；
2. 在线执行空库 `upgrade 0005`、受控 downgrade/再 upgrade，并核对 outbox、限流和审计触发器；
3. M1 执行 bootstrap、API、锁/并发、事务回滚和数据库中断/恢复测试；
4. D2 通过后，M2/M3 才接入真实身份/outbox 写端口；此前继续使用公开契约 Mock；
5. M7 另起变更实现 Windows Service/代理/备份恢复，M6 可并行实现 OpenAPI 客户端。

后续模块不得重复创建 readiness 聚合器、旧路由守卫或 outbox 写端口，也不得修改 `0001`～`0005`。M4 的 `OutboxClaimPort` 是尚未搭建的新端口，需由 M0 另行冻结。
