...
状态枚举、严格含义和升级条件只以 [SRS 第 1 节](../requirements/software-requirements-spec.md)为准，本计划不维护第二套定义。“原型代码存在”“待改造”“可选”仍分别只是资产、优先级和范围限定语；离线 Alembic SQL、SQL 编译、Mock、FastAPI `TestClient` 或单个函数测试只能支持其明确状态对象，不能外推为集成或完成证据。
...
| `domains/knowledge/{models,contracts,repository,service,read_port}.py` | 版本草稿/审核/发布、`EffectiveKnowledgeReadPort` | 消费 M1 审核身份；只在满足事件目录生产启用门禁的环境发布事件；供 M5 只读 |
...
| `domains/workflows/{models,contracts,repository,service,read_port}.py` | 流程版本/审核、`EffectiveWorkflowReadPort`、确定性匹配 | 只在满足事件目录生产启用门禁的环境发布事件；供 M5 只读 |
...
| Worker/索引迁移与 `test_m4_*` | 任务并发、宕机恢复、幂等消费 | 先依事件目录的提议 schema/样例实现 handler 和去重/重放；登记实际消费者并闭合契约后冻结事件，完成目录生产启用门禁后才在对应环境发布 |
