...
M0 当前冻结以下错误码：`HTTP_ERROR`、`VALIDATION_ERROR`、`DEPENDENCY_UNAVAILABLE`、`AUTHENTICATION_REQUIRED`、`FORBIDDEN`、`IDEMPOTENCY_KEY_REQUIRED`、`IDEMPOTENCY_CONFLICT`、`REQUEST_IN_PROGRESS`、`VERSION_CONFLICT`。M1 可在其设计记录中申请身份专用错误码，但不得改变这些既有含义。
