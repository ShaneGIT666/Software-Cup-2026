...
| 已冻结 | 生产者、实际消费者、payload、事务触发点、幂等、顺序/重复、失败恢复、权限/隐私和兼容测试均已登记 | 允许生产者和消费者按冻结版本完成实现与受控测试；不自动授权任何环境启用生产发布 |
...
“生命周期已冻结”和“允许在某一环境启用生产发布”是两个独立条件。事件只有同时满足以下条件，才算通过本目录所称的“生产启用门禁”：

1. 生命周期为“已冻结”，且已登记符合上文要求的实际消费者。
2. M0 ClaimPort 及消费者依赖的 claim、lease、retry、dead-letter/replay 和并发语义已按冻结契约实现。
3. 生产者/消费者契约测试通过，目标数据库上的提交、重复、乱序、失败恢复和重放集成验证通过。
4. 现行追踪矩阵已为该生产者—消费者状态对象登记相应集成证据和未关闭问题，启用环境、回滚/停发方式及责任边界已写入新的修改日志。

任一条件未满足时，可以继续开发、运行 Mock 或受控契约测试，但生产配置必须保持该事件发布关闭。如果解析、索引、缓存失效或其他业务一致性依赖该事件，依赖它的生产业务入口也必须保持关闭；不得跳过 outbox 后继续写入领域事实并把结果解释为同步降级。其他文档只引用“满足事件目录的生产启用门禁”，不得复制或缩减本清单。
...
| --- | --- | --- |
| M2/M3/M5 可对外观察的领域事实变化 | 普通 HTTP 使用 `CurrentUser` 派生的交互 actor；异步延续使用 M1 受管服务 actor 并保留 initiator | 仅当对应事件满足本目录生产启用门禁时，才在启用环境与业务状态、审计和必要幂等记录同事务追加 |
| M5 查询、回答、反馈等没有上述冻结事件的写入 | 同上；按安全规则记录审计或调用记录 | 不发布业务事件 |
| M1 用户、角色、密码等安全状态变化 | 经过认证的交互 actor | 仅当本目录存在满足生产启用门禁的安全事件时追加；不得为形式统一发布 |
...
没有实际消费者、幂等和回放语义的写操作不得为了“统一架构”发布 outbox。Mock、测试消费者、待建 M4 或表格中的预期消费者均不能满足生产启用门禁。
...
5. payload、主体、权限、兼容和实际消费者全部满足冻结条件后，本目录才把事件从“提议”改为“已冻结”；这一变化不自动提升功能实现状态，也不自动授权生产发布。
6. 消费链完成数据库、故障恢复和重放集成验证，并在追踪矩阵和新日志登记后，才满足第 1.1 节生产启用门禁，生产者方可在对应环境启用发布。
