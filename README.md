...
> 当前仓库仍处于开发和试点阶段。旧原型可以运行；截至 2026-08-17、记录 019 工作区，M0/M1 所列代码的实现状态为“单元已验证”。真实 PostgreSQL、前端认证接入、Windows Service、备份恢复和生产部署尚未完成。动态状态只以现行需求追踪矩阵为准。
...
下表是 2026-08-17、记录 019 工作区的摘要快照，后续状态变化只更新现行需求追踪矩阵和对应修改日志；仅当模块边界或 README 导航摘要需要调整时才修改本表。

| 模块 | 状态对象 | 实现状态 | 资产与证据限定 | 主要未完成项 |
| --- | --- | --- | --- | --- |
| 公共底座 M0 | 当前 M0 公共代码 | 单元已验证 | v1 响应/运行时及 OpenAPI 脱敏 500、请求 ID、严格环境枚举、CORS `ETag`、数据库 Session、强类型 readiness、旧表面保护、幂等和 outbox 写端口已有进程内/契约测试 | 真实 PostgreSQL 在线迁移/中断恢复、真实代理/跨域浏览器和生产部署验收 |
| 身份与审计 M1 | 本地账户与审计代码 | 单元已验证 | 会话、CSRF、用户/角色、审计、固定受管服务身份、bootstrap/activation 和 v1 路由已有进程内单元/API 测试 | PostgreSQL 触发器/锁/并发/回滚、前端接入和 OIDC |
...
生产写操作按用途分类，不得为了形式统一向所有内部状态更新发布 outbox。会改变可对外观察业务事实、并已在[事件目录](docs/design/event-catalog.md)中登记下游消费者的关键领域写操作，必须在同一数据库事务中提交：
...
- bootstrap 只允许在实例进入生产激活状态前，通过受控本地 CLI 在空交互用户库执行，并由固定 bootstrap 服务用户记账；首次管理员改掉临时密码并通过独立 activation CLI 激活后，bootstrap 永久拒绝再次执行。该生命周期已单元验证，真实数据库锁与并发仍待 D2 验收。
...
- 生产写事务必须按操作分类遵循业务状态、审计和必要幂等契约；只有事件目录已登记下游消费者的操作才追加 outbox。
...
2026-08-17、记录 019 的 D1.2 工作区复核结果为：

- 代码基线：提交 `e11ade9` 加记录 019 的工作区变更。
...
2. 在真实数据库上验收首次管理员 bootstrap、改密、activation、服务身份审计和生产 readiness 生命周期。
