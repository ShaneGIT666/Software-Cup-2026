...
当前代码已完成“模块 0：契约与数据底座”的第一步，并已搭建 M1 身份/审计基础内核和身份持久化接缝：包括 `/api/v1` 系统健康检查、请求 ID、统一 v1 错误/分页/并发契约、PostgreSQL/Alembic 基础，以及身份表、会话/权限依赖和登录限流服务。M1 尚未公开登录、用户或审计查询路由；旧 `/api` 与 JSON 原型业务链路暂时保留，知识、Worker 等领域尚未迁移。具体使用方式见 [后端说明](backend/README.md)。
