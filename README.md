...
当前代码已搭建“模块 0：契约与数据底座”的第一批代码，以及 M1 身份/审计基础内核和持久化接缝，包括 `/api/v1` 健康检查、请求 ID、统一 v1 错误/分页/并发契约、PostgreSQL/Alembic 基础、身份表、会话/权限依赖和登录限流代码。这些内容尚未完成真实 PostgreSQL 与 HTTP 业务闭环验证；M1 身份依赖还存在共享事务提交、授权快照和可信客户端地址门槛，不能视为认证功能已完成。M1 尚未公开登录、用户或审计查询路由；旧 `/api` 与 JSON 原型业务链路暂时保留。具体状态见 [后端说明](backend/README.md)。
