...
> 当前仓库仍处于开发和试点阶段。旧原型可以运行，M0/M1 公共底座代码已经搭建并通过进程内测试，但真实 PostgreSQL、前端认证接入、Windows Service、备份恢复和生产部署尚未完成。
...
| 状态 | 严格含义 |
| --- | --- |
| 原型代码存在 | 仓库中存在相应演示或旧实现代码；不表示满足目标需求 |
| 代码已搭建 | 已建立目标目录、接口、模型或基础实现，但不表示测试或集成已经通过 |
| 单元已验证 | 对应单元测试、契约测试或进程内测试已经通过 |
| 集成已验证 | 已连接声明的真实数据库、代理、浏览器、文件系统或 Provider 完成验证 |
...
| 模块 | 当前状态 | 主要未完成项 |
| --- | --- | --- |
| 公共底座 M0 | v1 响应、请求 ID、数据库 Session、readiness、旧表面保护、幂等和 outbox 写端口已搭建；进程内已验证 | 真实 PostgreSQL 在线迁移、中断恢复、真实代理和生产部署验收 |
...
| 配置 | 说明 |
| --- | --- |
| `APP_ENV` | 约定使用 `development`、`test` 或 `production` |
...
当前代码尚未对 `APP_ENV` 执行严格枚举校验。部署时必须使用上述约定值；在完成代码校验前，拼写错误可能导致生产保护没有按预期启用。
...
后端目前已经返回 ETag，但 CORS 暴露头暂时只有 `X-Request-ID`。跨域网页前端在 `ETag` 加入 CORS 暴露头并完成浏览器测试前，不能将该机制标记为跨域浏览器集成已完成。
...
按照当前 SRS，所有生产业务写操作必须在同一数据库事务中提交：
...
当前 M0 只完成 outbox 生产者公共写端口，M1 及后续领域尚未完成整个生产事务闭环。因此该条属于目标生产契约，不表示当前所有写路径已经满足。

如果后续决定某类生产写操作不需要 outbox，必须先修改 SRS 和公共契约，不能只在实现或 README 中使用“适用时”规避要求。
...
截至 2026-08-14，当前工作区复核结果为：

- 后端：`250 passed, 25 skipped`。
- 其中 3 项真实 PostgreSQL 测试因未配置专用 `_test` 数据库而跳过。
- 其余跳过项包含外部手册相关测试。
- Alembic 单一 head 为 `20260814_0005`。
- 离线 migration SQL 生成已有成功记录。
- 未执行真实 PostgreSQL 在线 upgrade/downgrade。
- 前端构建成功。
- 构建后的主 JavaScript 约为 1.05 MB，并触发大 chunk 警告。
- 浏览器认证与核心业务 E2E 尚未形成锁定、可复现的验收基线。
