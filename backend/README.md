...
M1 身份与审计基础内核及身份持久化接缝已经开始迁移；现有代码包含身份 Repository、登录限流/会话服务及当前用户/权限/CSRF 依赖，但没有公开登录、用户或审计查询路由。知识、文件、审核、Worker 和检索领域仍未迁移，因此当前 `/api` 原型继续使用 JSON 存储。不要把这些内部接缝视为认证或用户管理功能已经上线。
...
M1 本地账户基础依赖已包含 `argon2-cffi`。开发环境会话配置示例见仓库根目录 `.env.example`；启用 M1 登录或身份依赖前，生产环境必须提供独立的 `APP_AUTH_SECRET`，并使用 `Secure` 的 `__Host-` 会话 Cookie。M1 当前已完成身份/审计基础内核、数据库迁移和身份持久化接缝，登录与用户管理 HTTP API 仍未交付，不能将其标记为认证功能已上线。
