...
def validate_knowledge_file(file: UploadFile, content: bytes) -> str:
    ...
    if not suffix or suffix not in ALLOWED_KNOWLEDGE_TYPES:
        raise HTTPException(status_code=400, detail="资料入库仅支持 pdf、txt 和 md 文件")
    if not content:
            ...
            chunks.append(
                {
                    ...
                    "title": Path(file.filename or document_id).stem,
                    "sourceType": "document",
                    "sourceName": source_name or file.filename or document_id,
                    "page": page["page"],
