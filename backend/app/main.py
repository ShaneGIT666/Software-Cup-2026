...
async def multimodal_diagnosis(
    ...
    return ApiResponse(
        data={
            ...
            "answerMode": payload.get("answerMode", "insufficient_evidence"),
            "llmAnswerUsed": payload.get("llmAnswerUsed", False),
            "llmCandidateAccepted": payload.get("llmCandidateAccepted", False),
            "finalAnswerSource": payload.get("finalAnswerSource", "template"),
            "correctiveRag": payload.get("correctiveRag", {}),
            "safetyRules": payload.get("safetyRules", {}),
            "riskReviewRequired": payload.get("riskReviewRequired", False),
            "raw": payload,
        },
        message="故障图片诊断已完成",
    )
